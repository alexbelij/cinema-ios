...
public class ImportAndUpdateAction: MaintenanceAction {
  ...
  public init(library: MediaLibrary, movieDb: MovieDbClient, from url: URL) {
  ...
  public func performAction(completion: (ActionResult<[MediaItem]>) -> Void) {
    ...
    do {
      ...
      let updates = UpdateUtils.updates(from: schemaVersion, using: movieDb)
